...
class InstanceRegistry:
    def __init__(self, config: InstanceRegistryConfig):
        self._types = sorted(config.instance_types, key=lambda t: t.hourly_price_usd)

    @classmethod
    def from_yaml(cls, path) -> "InstanceRegistry":
        with open(path) as f:
    ...
    def candidates(self, min_ram_gb, min_vcpu):
...
def compute_k8s_capacity(instance, centroid_peak_rams, os_overhead_gb=2.0) -> K8SCapacityProfile:
...
def batch_max_jobs(instance, peak_ram_gb, declared_vcpu) -> int:
...
class NodeCostAccruer:
    ...
    @property
    def is_terminated(self): return self.termination_time >= 0

    def terminate(self, at_time):
...
class PoolCostSummary:
    ...
    cost_by_family: dict = field(default_factory=dict)
    cost_over_time: list = field(default_factory=list)
    node_count_over_time: list = field(default_factory=list)

    @classmethod
    def from_accruers(cls, accruers, sample_interval_s=60.0, sim_horizon=0.0):
